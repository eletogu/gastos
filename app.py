...
# Agregamos "Préstamo Personal" a la lista
CATEGORIAS_GASTOS = [
...
with col_reg1:
    with st.expander("➕ REGISTRAR GASTO, INGRESO O PRÉSTAMO", expanded=False):
        with st.form("nuevo_gasto", clear_on_submit=True):
            ...
            if st.form_submit_button("💾 Guardar Registro"):
                if con_g and mon_g > 0:
                    ...
                    texto_notif = f"✅ *Nuevo Registro*\n👤 {pag_g}\n📝 {con_g}\n💵 ${mon_g:,.0f} COP"
                    if cat_g == "Préstamo Personal": texto_notif = f"💸 *Préstamo Registrado*\n👤 {pag_g} prestó ${mon_g:,.0f} a su pareja."
                    enviar_notificacion(texto_notif)
                    st.rerun()

with col_reg2:
    with st.expander("🤝 REGISTRAR ABONO (Pagar deuda)", expanded=False):
...
# --- 👥 CUENTAS CLARAS (Hogar + Préstamos) ---
st.header(f"👥 Cuentas Claras ({mes_s_n})")

# 1. Gastos Compartidos (División 50/50)
categorias_hogar = [c for c in CATEGORIAS_GASTOS if c not in ["Carro - Ingresos", "Carro - Gastos", "Préstamo Personal", "Otro"]]
df_hogar = df_mes[df_mes['Categoría'].isin(categorias_hogar)]
t_leo_hogar = df_hogar[df_hogar['Pagador'] == 'Leonardo']['Monto'].sum()
t_cata_hogar = df_hogar[df_hogar['Pagador'].isin(['Cata', 'Esposa'])]['Monto'].sum()
deuda_hogar = (t_leo_hogar - t_cata_hogar) / 2

# 2. Préstamos Personales (100% Responsabilidad)
# Si Leonardo prestó, Cata le debe el total. Si Cata prestó, Leonardo le debe el total.
prestamos_leo = df_mes[(df_mes['Categoría'] == "Préstamo Personal") & (df_mes['Pagador'] == "Leonardo")]['Monto'].sum()
prestamos_cata = df_mes[(df_mes['Categoría'] == "Préstamo Personal") & (df_mes['Pagador'] == "Cata")]['Monto'].sum()
deuda_prestamos = prestamos_leo - prestamos_cata
...
deuda_total = deuda_hogar + deuda_prestamos + ajuste_abonos

b1, b2 = st.columns(2)
b1.metric("Gastos Hogar (Leo)", f"${t_leo_hogar:,.0f}")
b2.metric("Gastos Hogar (Cata)", f"${t_cata_hogar:,.0f}")

with st.container():
    if deuda_total > 0:
        st.info(f"💡 **Cata debe a Leonardo: ${deuda_total:,.0f} COP**")
    elif deuda_total < 0:
        st.info(f"💡 **Leonardo debe a Cata: ${abs(deuda_total):,.0f} COP**")
    else:
        st.success("✅ ¡Están totalmente a mano!")

with st.expander("🔍 Ver desglose de la deuda"):
    col_d1, col_d2 = st.columns(2)
    col_d1.write(f"🏠 Por gastos del hogar: **${abs(deuda_hogar):,.0f}**")
    col_d1.write(f"💸 Por préstamos directos: **${abs(deuda_prestamos):,.0f}**")
    col_d2.write(f"🤝 Total abonado este mes: **${(abonos_cata_a_leo + abonos_leo_a_cata):,.0f}**")
    
    df_abonos_mes = df_mes[df_mes['Categoría'].isin(["Abono a Deuda", "Préstamo Personal"])].copy()
    if not df_abonos_mes.empty:
        df_abonos_mes['Fecha'] = df_abonos_mes['Fecha'].dt.strftime('%d/%m/%Y')
        st.dataframe(df_abonos_mes[['Fecha', 'Pagador', 'Categoría', 'Concepto', 'Monto']], use_container_width=True, hide_index=True)
...
st.plotly_chart(px.pie(df_mes[~df_mes['Categoría'].isin(["Abono a Deuda", "Préstamo Personal"])], values='Monto', names='Categoría', hole=0.5, title="Distribución de Consumo Real"), use_container_width=True)