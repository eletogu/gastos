...
# --- LÓGICA DE CÁLCULO (Función que entrega desgloses) ---
def obtener_detalles_balance(dataframe):
    ...
    deuda_hogar = (t_leo_h - t_cata_h) / 2
    ...
    ajuste_abonos = a_leo - a_cata
    
    return deuda_hogar, deuda_prestamos, ajuste_abonos, (t_leo_h, t_cata_h, p_leo, p_cata, a_leo, a_cata)

# Cálculos
fecha_inicio_mes = datetime(anho_s, mes_s_num, 1)
df_ant = df_clean[df_clean['Fecha'] < fecha_inicio_mes]
dh_ant, dp_ant, ab_ant, _ = obtener_detalles_balance(df_ant)
saldo_anterior = dh_ant + dp_ant + ab_ant

dh_tot, dp_tot, ab_tot, raw = obtener_detalles_balance(df_clean)
saldo_total = dh_tot + dp_tot + ab_tot

# --- 👥 SECCIÓN DE DEUDAS ---
st.header(f"👥 Cuentas Claras")
c_m1, c_m2 = st.columns(2)
c_m1.metric("Viene del pasado", f"${abs(saldo_anterior):,.0f}", help="Deuda acumulada antes de este mes")
...
# --- 🔍 AQUÍ ESTÁ EL DESGLOSE QUE FALTABA ---
with st.expander("🔍 VER DETALLE DE LA DEUDA (Toda la historia)"):
    st.write("A continuación verás de dónde sale el saldo total acumulado:")
    ...
    df_hist = df_clean[df_clean['Categoría'].isin(["Préstamo Personal", "Abono a Deuda"])].tail(10).iloc[::-1]
    if not df_hist.empty:
        df_hist_d = df_hist.copy()
        df_hist_d['Fecha'] = df_hist_d['Fecha'].dt.strftime('%d/%m/%Y')
        st.dataframe(df_hist_d[['Fecha', 'Pagador', 'Categoría', 'Concepto', 'Monto']], use_container_width=True, hide_index=True)
...
with col_c1:
    ...
    ing, gas = df_c_m[df_c_m['Categoría'] == "Carro - Ingresos"]['Monto'].sum(), df_c_m[df_c_m['Categoría'] == "Carro - Gastos"]['Monto'].sum()
...
with col_c2:
    ...
    for i, (cat, lim) in enumerate(PRESUPUESTOS_FAMILIARES.items()):
        ...
        cp[i].write(f"**{cat}**")
        cp[i].progress(min(gast/lim, 1.0))
        cp[i].caption(f"${gast:,.0f}/${lim:,.0f}")
